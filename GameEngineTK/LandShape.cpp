...
bool LandShape::IntersectSphere(const Sphere& sphere, Vector3* reject)
{
	...
	bool hit = false;
	// 大きい数字で初期化
	float over_length = 1.0e5;	/* 二番目にめり込んでるやつが、どれくらいめりこんでるのかのメモ用 */
	Vector3 l_inter;
	Vector3 l_normal;
	Vector3 l_down;
	// スケールを取得
	...
	localsphere.Center = Vector3::Transform(sphere.Center, m_WorldLocal);	/* 詳しい計算はUpdate? */
	// 半径をワールドをワールド座標系からモデル座標系に変換	/* ワールドから戻すので、スケールで割る */
	...
	for (int i = 0; i < nTri; i++)
	{
		...
		// 三角形と球の当たり判定
		if (CheckSphere2Triangle(localsphere, tri, &temp_inter))	/* どれかの三角形にヒットしたら */
		{// ヒットした
			hit = true;
			// 衝突点から球の中心へのベクトル
			Vector3 sub = localsphere.Center - temp_inter;
			// 球の中心が三角形にめりこんでいる距離を計算
			temp_over_length = sub.Dot(-tri.Normal);	/* tri.Normalはめり込む方向。その逆に押し出している */

			// めりこみ具合がここまでで最小なら
			if (temp_over_length < over_length)
			{
				// ヒット座標、法線、めりこみ距離を記録		/*  一番浅い記録を残している。複数個の三角形とヒットしてる時用で、一個ならそれがそのまま */
				l_inter = temp_inter;
				l_normal = tri.Normal;
	...
	if (hit)
	{
		// 距離をモデル座標系からワールド座標系での長さに変換	/* 再びスケールをかけ合わせ、ワールド座標に戻す */
		over_length *= scale;
...
bool LandShape::IntersectSegment(const Segment& segment, Vector3* inter)
{
	...
	const float limit_angle = XMConvertToRadians(30.0f);	/* あとで#defineしとくといい。制限角度 */
	Vector3 l_inter;
	...
	for (int i = 0; i < nTri; i++)
	{
		...
		// 上方向ベクトルと法線の内積
		// 長さが１の、二つのベクトル同士の内積は、コサイン（ベクトルの内積の定義）より
