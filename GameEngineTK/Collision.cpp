...
/// <param name="_closest">最近接店の座標（出力）</param>
/// <param name="_triangle">三角形（結果出力用）</param>
void ClosestPtPoint2Triangle(const Vector3& _point, const Triangle& _tri, Vector3* _closest)
{
	...
	float d1 = P0_P1.Dot(P0_PT);	/* ベクトルAの目的地 */
	float d2 = P0_P2.Dot(P0_PT);	/* ベクトルBの目的地 */

	if (d1 <= 0.0f && d2 <= 0.0f)	/* 2ベクトルの角度が90度以下ならば */
	{
		// P0が一番近い
		*_closest = _tri.P0;
		return;
	...
	// 2===================================================================================
	// _pointがP1の外側の頂点領域にあるかチェック（つまりP1が一番近いかどうか）
	Vector3 P1_PT = _point - _tri.P1;

	float d3 = P0_P1.Dot(P1_PT);	/* ベクトルCの目的地 */
	float d4 = P0_P2.Dot(P1_PT);	/* ベクトルDの目的地 */

	if (d3 >= 0.0f && d4 <= d3)	/* 内積の値が大きい＝角度の開きが大きいなら */
	{
		// P1が一番近い 　　　……なんでや？　コレに当てはまらないなら、少なくともP1が最近接ではない
		*_closest = _tri.P0;
		return;
	}
	...
	// 3===================================================================================
	// _pointがP0_P1の辺領域の中にあるかどうかチェックし、
	// あるなら_pointのP0_P1上に対する射影を返す
	float vc = d1 * d4 - d3 * d2;

//	//* こっからGithub->GameEngineのコピペ
...
bool CheckSphere2Triangle(const Sphere& _sphere, const Triangle& _tri, Vector3* _inter)	/* _interだけは外部に渡す情報なので参照じゃダメ */
{
	//	//*　ヤッパコピペ
	Vector3 p;

	// 球の中心に一番近い点である、三角形上にある点pを見つける
